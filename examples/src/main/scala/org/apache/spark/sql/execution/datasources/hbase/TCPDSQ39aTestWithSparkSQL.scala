...
object TCPDSQ39aTestWithSparkSQL {
  def main(args: Array[String]){
    val spark = SparkSession.builder()
      .appName("TCPDS_Q39a_TestingWithSparkHive")
      .enableHiveSupport()
      .getOrCreate()
    ...
    val ret = sql("WITH inv AS (SELECT w_warehouse_name, w_warehouse_sk, i_item_sk, d_moy, stdev, mean, " +
      ...
      "SELECT inv1.w_warehouse_sk, inv1.i_item_sk, inv1.d_moy, inv1.mean, inv1.cov, inv2.w_warehouse_sk, " +
      "inv2.i_item_sk, inv2.d_moy, inv2.mean, inv2.cov FROM inv inv1, inv inv2 " +
